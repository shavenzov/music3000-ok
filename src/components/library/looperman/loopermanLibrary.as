...
private var collection : AsyncLoopermanCollection;
...
private function _search( updateParams : Boolean = true ) : void
{
	...
	var searchParams : Object = { genres : selectedGenres, categories : selectedCategories, tempoFrom : tempo.values[ 0 ], tempoTo : tempo.values[ 1 ], 
		                          keys : selectedKeys, orderBy : sortList.sortField, order : sortList.descending ? 'desc' : 'asc'
	                            };
	...
	if ( updateParams )
	{
		var tagParams : Object = { tempoFrom : tempo.values[ 0 ], tempoTo : tempo.values[ 1 ] };
...
{
	collection.search( {  orderBy : sortList.sortField, order : sortList.descending ? 'desc' : 'asc' } );
}
