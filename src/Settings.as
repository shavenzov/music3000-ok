...
package 
{
	...
	public class Settings
	{
		...
		/**
		 * Минимальная длина микса для публикации в фреймах 
		 */		
		public static const MIN_PUBLICATION_LENGTH : Number = 44100 * 5;
		...
		/**
		 * Корень песочницы приложения
		 *  
		 */
		public static var SANDBOX_URL : String = '';
		...
		public static var APPLICATION_SUPPORT : String = 'odnoklassniki.ru/group/52091635695706';
		...
		public static function parseSettings( xml : XML ) : void
		{
			if ( xml.help != undefined )
			{
				...
				if ( xml.help.support != undefined )
				{
					APPLICATION_SUPPORT = resolveProtocol( xml.help.support.text() );
				}
			}
			
			AMF_HOST = resolveProtocol( xml.apiURL.text() );
			
			if ( xml.sandboxURL != undefined )
			{
				SANDBOX_URL = resolveProtocol( xml.sandboxURL.text() ) + '/';
			}
			...
			{
				LOOPERMAN_CATEGORIES.push( { id : item.@id.toString(), icon : SANDBOX_URL + 'icons/' + item.@icon.toString(), short : ( ( item.@short == undefined ) ? item.text() : item.@short.toString() ), label : item.text() } );
			}
