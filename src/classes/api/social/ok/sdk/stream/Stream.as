package classes.api.social.ok.sdk.stream 
{
	...
	public class Stream 
	{
		...
		{
			_callback = callback;
			if(!OKApi.hasEventListener(ApiCallbackEvent.CALL_BACK)){
				OKApi.addEventListener(ApiCallbackEvent.CALL_BACK, onCallBack);
			}
			...
			_request = OKApi.getSignature(_request, true);
			OKApi.showConfirmation("stream.publish", confirmation_message, _request.sig);
		}
			...
			if (event.method == "showConfirmation") {
				switch (event.result) {
					case "ok":
						...
						OKApi.callRestApi("stream.publish", onStreamPublish, _request);
						break;
					case "cancel":
