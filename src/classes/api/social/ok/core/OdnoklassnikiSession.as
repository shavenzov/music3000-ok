package classes.api.social.ok.core
{
	...
	public class OdnoklassnikiSession 
	{
		...
		private function handleOutStatus(event : StatusEvent) : void
		{
			...
			{
				OKApi.getInstance().dispatchEvent(new ApiServerEvent(ApiServerEvent.CONNECTION_ERROR));
			}
		}
		...
		private function handleOutStatusReconnect(event : StatusEvent) : void
		{
			if (event.level != StatusEvent.STATUS)
			{
				OKApi.getInstance().dispatchEvent(new ApiServerEvent(ApiServerEvent.PROXY_NOT_RESPONDING));
			}
		}
		...
		public function establishConnection() : void
		{
			...
			if (!this._is_connected)
			{
				...
				OKApi.getInstance().dispatchEvent(new ApiServerEvent(ApiServerEvent.CONNECTED));
			}
		}
		
		public function apiCallBack(methodName : String, result : String, params : String) : void
		{
			OKApi.getInstance().dispatchEvent(new ApiCallbackEvent(ApiCallbackEvent.CALL_BACK, false, false, methodName, result, params));
		}
